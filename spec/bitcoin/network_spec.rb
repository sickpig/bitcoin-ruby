...
describe 'Bitcoin::network' do
  ...
  it 'returns network descriptor' do
    ...
    net = Bitcoin::network
    net[:magic_head].should == "\xE3\xE1\xF3\xE8"
  ...
  it 'uses correct magic_head when parsing a message' do
    pkt = ["e3 e1 f3 e8 69 6e 76 00 00 00 00 00 00 00 00 00 49 00 00 00 11 ea 1c 91 02 01 00 00 00 e0 41 c2 38 f7 32 1a 68 0a 34 06 bf fd 72 12 e3 d1 2c b5 12 2a 8c 0b 52 76 de 82 30 b1 00 7a 42 01 00 00 00 33 00 09 71 a9 70 7b 6c 6d 6e 77 aa 2e ac 43 f3 e5 67 84 cb 61 b2 35 fb 8d fe e0 86 8b 40 7c f3".split(" ").join].pack("H*")
  ...
  it 'uses correct magic head when creating a message' do
    Bitcoin::network = :testnet
    Bitcoin::Protocol.pkt('foo', "bar")[0...4].should == "\xDA\xB5\xBF\xFA"

    Bitcoin::network = :bitcoin
    Bitcoin::Protocol.pkt('foo', "bar")[0...4].should == "\xE3\xE1\xF3\xE8"
  end
